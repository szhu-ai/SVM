function [ bac_raw, weight_raw, bac_perm_label, weight_perm_label, bac_perm_trial, weight_perm_trial, C_cv] = svm_main_oneout(s1,s2,norm_type,nfold,permute_label,permute_trial,nperm,C_range)
...
%% inputs: 
% "s1" and "s2" are spike counts in condition -1 (s1) and condition 1 (s2), of the form (number of trials x number of neurons)
...
ntrain1=n1-1; % number of trials for training from condition 1
ntrain2=n2-1; % number of trials for training from condition 2
...
label_test=cat(1,0-ones(n1-ntrain1,1),ones(n2-ntrain2,1));    % labels validation
...
%     fprintf(['cross validation repeat',num2str(idx_cv),'/',num2str(ncv),'\n'])    
    ...
    rp1=[setdiff(1:n1,rp(idx_cv,1)),rp(idx_cv,1)];
    rp2=[setdiff(1:n2,rp(idx_cv,2)),rp(idx_cv,2)];
    ...
    s1_test=s1(rp1(ntrain1+1:end),:);     % take remaining trials for testing
    s2_test=s2(rp2(ntrain2+1:end),:);
    s_test=cat(1,s1_test,s2_test);     % concatenate validation data in condition 1 and 2 
